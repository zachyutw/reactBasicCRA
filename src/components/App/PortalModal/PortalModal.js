...
class PortalModal extends Component {
    ...
    componentDidMount(){
        const {scrolling=false} = this.props;
    ...
    verticalHeight = (vertical)=>{
        switch(vertical){
        case "toptop": return "2vh";
        case "top": return "5vh";
        case "normal": return "25vh";
        case "bottom": return "75vh";
        default: return "25vh";
        }
    }  
    render(){
    let {hide,children,closeBtn=false,vertical="normal",style,size, close=<Icon className={s.closeIcon}  size='large' name="times"/> } = this.props;
    return <div className={s.bgCurtain} > 
                    <Segment className={s.segment+" "+s.modal} style={{width:this.sizing(size),top:this.verticalHeight(vertical),...style}}>
